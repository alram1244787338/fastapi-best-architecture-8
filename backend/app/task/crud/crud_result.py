...
class CRUDTaskResult(CRUDPlus[TaskResult]):
    ...
    def _build_conditions(
        ...
        if start_date is not None:
            start_dt = datetime(start_date.year, start_date.month, start_date.day, tzinfo=timezone.tz_info)
            conditions.append(self.model.date_done >= start_dt)
        if end_date is not None:
            # 包含截止日期当天 23:59:59.999999
            end_dt = datetime(
                end_date.year, end_date.month, end_date.day, 23, 59, 59, 999999, tzinfo=timezone.tz_info
            )
    ...
    async def get_recent_executions(
        ...
        stmt = select(self.model).where(*conditions).order_by(self.model.id.desc()).limit(limit)
        result = await db.execute(stmt)
